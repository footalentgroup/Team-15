        ...
        <div className="flex gap-5 justify-end h-12">
            {["Tareas", "Exámenes", "Actitudinal", "Asistencia"].map((label, index) => (
                <Link href={links[index]} key={index}>
                    <button
                        type="button"
                        className={`min-w-[155px] min-h-12 ${pathname === links[index] ? "bg-yellow-500 text-black" : "bg-white text-black"
                            } border-2 border-black font-semibold text-sm px-4 rounded-md filter drop-shadow-general`}
                    >
                        {label}
                    </button>
                </Link>
